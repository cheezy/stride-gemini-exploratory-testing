...
This skill produces good charters and enumerates candidate targets. It does not run the session (that's the `session` skill) or generate the probes (that's `heuristics`) — it decides what the session is *about*. It is reached from the orchestrator skill and drives the native Gemini CLI TOML commands `/charter` (`commands/charter.toml`) and `/nightmare-headline` (`commands/nightmare-headline.toml`).
...
That is: **Explore `<target>` with `<resources>` to discover `<information>`.**
...
This is the risk-driven engine behind the `/nightmare-headline` command (`commands/nightmare-headline.toml`).

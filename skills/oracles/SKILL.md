...
When you run a probe end to end — the `/explore` TOML command (`commands/explore.toml`) drives the running app one `run_shell_command` at a time — you apply these oracles to each result it returns.

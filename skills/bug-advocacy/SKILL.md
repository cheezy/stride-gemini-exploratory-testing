...
Translate the failure out of the product's vocabulary and into a person's. Not "the tenant scope is missing from the import query" but "a customer importing their own export can pull another customer's records into their board, and neither of them can tell."
...
The three bugs from the extension's example session sheet and example debrief (`fixtures/`), rated against the ladder. (The Critical row is an added illustration — no bug in that session rated Critical.)

| Bug | Worst demonstrated failure | Ladder clause matched | Modifiers | Level |
|---|---|---|---|---|
| BUG-1 — malformed CSV silently truncates the import | A 40-row file imported 13 rows and reported "Import complete"; rows 15–40 were well-formed and were dropped without a signal | High — *a failed operation reporting success about work the product accepted as well-formed*, and *valid data lost with the affected records still identifiable* | Reach: generalized to any parse error, not just the unclosed quote — yes. Avoidability: the user cannot tell it happened, so cannot prevent or repair it — yes. Persistence: silent, and the incomplete board outlives the import — yes. **3 of 3, and it still stays High: rule 4 says modifiers never produce a Critical, and no Critical ladder clause matches** | **High** |
| BUG-3 — import is not idempotent; a duplicate upload doubles the board | Re-importing the same valid 30-row file created 30 duplicate cards with no dedupe and no prompt | High — *valid data persisted wrong, affected records still identifiable* | Reach: reproduced for every file tried and on every repeat — yes. Avoidability: the user can see the duplicates and delete them — no. Persistence: wrong state persists but is plainly visible — no. **1 of 3, and one modifier never moves a rating (rule 2)** | **High** |
| BUG-2 — no progress feedback on large imports | ~40s of frozen UI on a 12 MB file, indistinguishable from a hang; the import then succeeded correctly | Moderate — *behavior wrong or misleading while the user works, where no wrong state survives* | none demonstrated | **Moderate** |
| *(illustrative)* Import accepts a CSV exported from another tenant's board | `<tenant B>`'s exported rows accepted into `<tenant A>` with no tenant check | Critical — *data crossing a boundary that must contain it* | irrelevant; Critical is the ceiling | **Critical** |

Three calibration notes, because these are the disagreements that actually happen:

- **Why the silent truncation is High and not Minor.** The malformed row itself is input the product could not have interpreted — that clause is Minor, and it is the one a quick read reaches for. But rows 15–40 were **well-formed** and were dropped anyway, and the UI reported success about the file it accepted. What the product accepted as well-formed is the line: had the only casualty been the one unclosed-quote row, this would be Minor.
- **Why the duplicate import is High and not Moderate.** Moderate requires that nothing incorrect survives. Thirty duplicate cards survive the action and sit in the user's board until someone removes them by hand. That the user can *see* them is why no modifier aggravates it — visibility is not correctness.
- **Why the frozen UI is Moderate and not High.** Nothing was lost, nothing was reported falsely, and the data after the import is right. The cost is confusion and a real chance the user kills the tab — a degraded workflow, which is the Moderate clause verbatim. Do not import the user's alarm into the rating.
...
**Every one of these fields carries observed system output, so the redaction rule applies to all of them.** `minimal_repro`, `worst_observed`, `generalization`, and `stakeholder_impact` must contain no real credentials, tokens, session identifiers, customer data, or internal hostnames — use placeholders (`<tenant A>`, `<redacted token>`, `example.com`), exactly as everywhere else in a session's output. A repro that only works with a real secret pasted into it is a repro that must be rewritten to name where the secret lives instead. This binds hardest on anything written to `.exploratory/` (see the `session` skill): a file outlives the session and can be read by someone who never saw the run.

## Handing off

The `oracles` skill decides *whether* a result is a defect; this skill decides what that defect looks like by the time anyone reads it, and how bad it is. The finished write-up flows into the session notes and the debrief (the `session` skill), and the `explorer` custom agent emits it as a `bugs[]` entry — `minimal_repro`, `worst_observed`, `generalization`, and `stakeholder_impact` are the four RIMGEA products, and `severity` is the rating.

**Isolate repays its cost twice.** An isolated minimal repro is a test case that has not been written down yet — the shortest set of conditions that still triggers the failure is precisely what a minimal test case is. Once in the report a developer can act on, and again the moment anyone turns that bug into a regression check. It is also why `"not established: …"` matters rather than embarrasses: a bug whose repro was never isolated is one that will be honestly reported as not convertible, instead of having a trigger guessed for it.

Two of the steps feed work back upstream rather than into the report. **Generalize** often reveals a whole class of failure worth its own mission — that goes to `chartering` as a candidate charter, and onto `.exploratory/backlog.md`. And an **unverified escalation** from Maximize goes to the off-charter parking lot for the same reason: the next session demonstrates it or rules it out, and only then does it touch a severity.
